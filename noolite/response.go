...
func (r *Response) GetDeviceState() StatusType {
	if r.CheckCRC() && r.Ctr == CtrResponseSuccess &&
	...
	} else if r.CheckCRC() && r.Mode == ModeNooliteRX && r.Ctr == CtrResponseSuccess {
		switch r.Cmd {
		case CmdSensTempHumi:
			switch r.Fmt {
		...
		case CmdBatteryLow:
			return NewBatteryLowStatusStatus(true, [4]byte{r.ID0, r.ID1, r.ID2, r.ID3})
		case CmdOn:
